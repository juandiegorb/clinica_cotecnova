<?php
	...
	$datos = $mysql->efectuarConsulta("SELECT citas.id_cita, citas.fecha_hora, citas.motivo_consulta, CONCAT(usuarios.nombre_completo, ' ',usuarios.apellidos) AS 'paciente', 
	CONCAT(medicos.nombre_completo, ' ',medicos.apellidos) AS 'medico' FROM citas INNER JOIN usuarios ON usuarios.id_usuario = citas.usuario_id INNER JOIN medicos ON medicos.id_medico = citas.medico_id WHERE citas.estado = 1");
	...
	$objPHPExcel->getProperties()->setCreator("Natalia Agudelo")->setDescription("Reporte de citas caducadas");
	
	//Establecemos la pestaña activa y nombre a la pestaña
	$objPHPExcel->setActiveSheetIndex(0);
	$objPHPExcel->getActiveSheet()->setTitle("Citas_caducadas");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE CITAS CADUCADAS');
	...
	$values = new PHPExcel_Chart_DataSeriesValues('Number', 'Citas!$D$7:$D$'.$fila);
	
	// definir origen de los rotulos
	$categories = new PHPExcel_Chart_DataSeriesValues('String', 'Citas!$B$7:$B$'.$fila);
